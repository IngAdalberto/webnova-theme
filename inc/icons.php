<?php
/**
 * Iconos SVG para el tema WebNova.
 *
...
/**
 * Retorna el SVG inline de un ícono específico.
 *
 * @param string $name Nombre del ícono.
 * @param array $args Argumentos opcionales (clase, tamaño).
 * @return string HTML del ícono SVG.
 */
function webnova_theme_get_icon(string $name, array $args = []): string
{
    $icons = [
    ...
    $class = $args['class'] ?? 'webnova-icon webnova-icon--' . $name;
    $size = $args['size'] ?? 24;
    $stroke_width = $args['stroke_width'] ?? 2;

    $svg = sprintf(
        '<svg xmlns="http://www.w3.org/2000/svg" width="%d" height="%d" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="%d" stroke-linecap="round" stroke-linejoin="round" class="%s">%s</svg>',
        esc_attr($size),
        esc_attr($size),
